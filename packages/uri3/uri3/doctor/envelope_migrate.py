...
def migrate_workflow_log(path: Path) -> dict[str, int]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if event.get("type") == "WorkflowCompleted":
            missing = [field for field in ("workflow_status", "execution_status", "service_result_status") if field not in event]
            if missing:
                workflow_status, execution_status, service_result_status = derive_statuses(bool(event.get("ok")))
...
def migrate_workflow_logs(root: Path) -> dict[str, object]:
    ...
    return {"ok": True, "logs": len(list(logs_dir.glob("*.jsonl"))), "updated_events": updated_events, "files": files}
