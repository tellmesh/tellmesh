...
def call_uri_graph_backend(
    ...
    *,
    ...
    path = _resolve_path(graph_path, context)
    if not path.is_file():
        return service_result(
            ok=False,
            result_type="error",
            errors=[{"code": "GRAPH_NOT_FOUND", "detail": f"Graph file not found: {path}"}],
        )
    ...
    try:
        ...
        return service_result(
            ok=False,
            result_type="error",
            errors=[{"code": "GRAPH_LOAD_FAILED", "detail": str(exc)}],
        )
    ...
    if errors:
        return service_result(
            ok=False,
            result_type="error",
            errors=[{"code": "GRAPH_INVALID", "detail": "; ".join(errors[:5])}],
        )
