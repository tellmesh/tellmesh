...
def write_domain_pack(
    ...
    *,
    ...
    handler_dir = root / "domains" / model.domain_id / "handlers"
    files["handler"] = write_file(handler_dir / handler_name, handler_source)
    write_file(root / "domains" / model.domain_id / "__init__.py", "")
    write_file(handler_dir / "__init__.py", "")

    agent_path = root / "contracts" / "agents" / f"{model.agent['id'].replace('-', '_')}.yaml"
    ...
    registry_ref = {
        "domain_pack": str(out_dir.relative_to(root)) if out_dir.is_relative_to(root) else str(out_dir),
        "agent_contract": str(agent_path.relative_to(root)),
    }
...
def generate_domain_pack_from_tree(tree: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    merge_main_contracts(model.domain_id, resources, views, proto_text)
    return files


def generate_domain_pack(uri_tree_path: str | Path, domain_dir: str | Path) -> dict[str, str]:
    tree = parse_uri_tree(uri_tree_path)
    return generate_domain_pack_from_tree(tree, Path(domain_dir))
